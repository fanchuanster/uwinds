...
public class Assignment {
	...
	static private void testBSTree()
	{
		System.out.println("testBSTree start...");
		...
		long totalTimeAdd = System.currentTimeMillis( ) - startTime;
		...
		{
			int res = bst.get(i);
			assert res > 0 : "Failed to get " + i;
		}
		long totalTimeGet = System.currentTimeMillis( ) - startTime;
		
		startTime = System.currentTimeMillis();
		int from = 1;
		IntStream.range(1, to+1).map(i -> to - i + from).forEachOrdered(n -> {
			bst.remove(n);
		});
		long totalTimeDeletion = System.currentTimeMillis( ) - startTime;
		
		System.out.format("add %d\tget %d\tdelete %d%n", totalTimeAdd,totalTimeGet, totalTimeDeletion);
